...
-   Note: we are only analysing the treatment effect of either fish oil
    or vitamin D, not both. \## Age distribution
...
-   VITAL:
    $\small\text{pain_yr4}_i = \beta_0 + \beta_1\text{fishoilactive}_i + \beta_2 \text{vitdactive}_i + \beta_3 \text{pain_base}_i + \varepsilon_i$
...
-   VITAL:
    $\small\text{pain}_{ij} = \beta_0 + \beta_1  \text{fishoil}_i + \beta_2 \text{time}_{ij} + \beta_3(\text{fishoil}_i \text{time}_{ij}) + \beta_4  \text{vitd}_i + \beta_5 (\text{vitd}_i \text{time}_{ij}) + \beta_6 \text{pain_base}_i + b_{0i} + \varepsilon_{ij}$
