...
-   Here we used predictive mean method for the imputation with m=5
...
- It is crucial to at least make sure we are using statistical principled method to quantify our uncertainties. Both in the imputation part and parameter estimation part (We could use bad imputation method for MI)
- The expertise input can be an important on how we handle the missing data (which estimand? can we use LOCF?)
- Different method can be more beneficial than others depends on dataset (LME better for VITAL as more time poins, imputation better for ACU as no covariates missing)
