...
- Missing Completely at Random (MCAR) - Missing data is independent of the study. This can occur due to illness/death, a treatment malfunction - nothing to do with the results of the research. This is the only verifiable missingness mechanism. 
- Missing at Random - Uncertainty as to how the data is missing. Data is missing not due to the observed values but potential variables in the study. 
- Missing Not at Random - Data is informatively missing. This is due to the unobserved data itself. Participants in a study missing their appointments or withholding a true response is an example of data being missing not at random. This is the most difficult to account for. We must investigate the data distributions of patients with missing data and compare to full data distributions. 

# Project Aims
This project investigates missing data analysis methods in two longitudinal RCT data sets. As the first part of the project, we are visualizing the proportion and patterns of missing values for each data set. The trial are summarised in the following table: 

| Trial                  | Vitamin D & Fish Oil Trial   | Acupuncture Trial           |
| -----------------------| -----------------------------| ----------------------------|
| Sample Size            | 20,000+                      | 401                         |
| Trial                  | Longitudinal RCT             | Longitudinal RCT            |
...
data_miss %>%
  ...
  stat_identity(geom = "text", colour = "white", size = 5,
                aes(label =perc_pat), 
                position = position_stack(vjust=0.5)) +
  scale_fill_manual(name="",values=c("olivedrab", "lawngreen"), labels=c("Missing", "Measured"))+
  scale_x_discrete(name = "Time", labels = c("base" = "Baseline", "yr1" = "Year 1", "yr2" = "Year 2", "yr3" = "Year 3", "yr4"="Year 4")) + 
  scale_y_continuous(name = "Number of Patients") +
  theme(text = element_text(size = 24),
        axis.text.x = element_text(angle = 0, vjust = 0.5), legend.position = "top")
)
```
...
Figure3. The missing data patterns (where 0 represents missing data and 1 represents observed data) between the groups (Fish Oil vs. Placebo) are displayed. Some differences in the patterns can be observed, suggesting that the data may not be missing completely at random. This observation warrants further investigation in the subsequent steps."
...
```{r mytable4, echo=FALSE, warning=FALSE, fig.height=6, fig.width=14}
pct_format = scales::label_percent(accuracy = .1)
...
Figure 4. Proportions of Missing data over time in longitudinal trial (Vickers et al. 2004)

```{r mytable5, echo=FALSE, warning=FALSE, fig.height=6, fig.width=14}
acupuncture_miss %>%
...
Figure 5. Proportions of missing data(dark green) within treatment groups over time

```{r mytable6, echo=FALSE, warning=FALSE, fig.height=6, fig.width=14}
...
Figure 6. Missing data (dark green) pattern over time

####

# Common Analysis for Data with Missing Observations [^3]

1. Complete Case Analysis: Only includes participants with no missing data in the analysis, ignoring cases with any missing values. This method assumes the data is Missing Completely at Random. 
2. Simple Imputation: Replaces missing values with a single estimate, such as: Last Observation Carried Forward (LOCF) & Baseline Observation Carried Forward (BOCF).
...
4. Mix-effect model:Uses likelihood-based methods to estimate parameters while accounting for missing data. This method assumes the data is Missing at Random 
5. Multiple Imputation:Replaces missing data with multiple plausible values, generating multiple datasets for analysis and combining the results for inference. This method assumes the data is Missing at Random. 

# Next step

To address this issue, we must conduct a sensitivity analysis on the data set. This begins with identifying which of the missing mechanism the missing data in the data sets are under. This can be done by investigating patterns in the data set.  

We will proceed to use a mixed-effects model as the primary method for analyzing both datasets. For the Acupuncture dataset, multiple imputation method may be beneficial due to the limited number of data collection time points.

Although the original methods for handling missing data are unknown, we will also apply a simpler approach, such as Last Observation Carried Forward (LOCF), as a less statistically rigorous method to the datasets. Which unfortunately is more commonly used method than it should have been. We will compare the outcomes across methods to evaluate the extent to which the choice of missing data handling impacts our conclusions for both datasets
...
[^Github]: The code and datasets for this project can be viewed at our GitHub repository here: <https://github.com/amydebrun/Missing-Data.git>
