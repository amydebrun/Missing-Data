...
# Project Aims
This project investigates missing data analysis methods in two longitudinal RCT data sets. As the first part of the project, we are visualizing the proportion and patterns of missing values for each data set. The trials are summarised in the following table: 
...
We must conduct a sensitivity analysis of the data set to address this issue. This begins with finding which of the missing mechanisms the missing data in the data sets are under. This can be done by investigating patterns in the data set. 

We will use a mixed-effects model as the primary method for analysing both datasets. For the Acupuncture dataset, the multiple imputation method may be beneficial due to the limited number of data collection time points.

Although the original methods for handling missing data are unknown, we will also apply a simpler approach, such as Last Observation Carried Forward (LOCF), as a less statistically rigorous method to the datasets. Which unfortunately is a more commonly used method than it should have been. We will compare the outcomes across methods to evaluate how much the choice of missing data handling affects our conclusions for both datasets.
