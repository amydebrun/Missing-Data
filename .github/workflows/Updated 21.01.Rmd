...
```
Figure1. The proportions of missing data (represented in dark green) at different time points. The observed missing data pattern is non-monotonic.
...
Figure2. Proportions of missing data (dark green) over time in different groups (Fish Oil vs. Placebo) are shown. Similar proportions of missing data were observed between the two groups.
...
Figure4. Proportions of Missing data over time in longitudinal trial (Vickers et al. 2004)
...
Figure5. Proportions of missing data(dark green) within treatment groups over time
...
Figure6. Missing data (dark green) pattern over time
...
5. Multiple Imputation:Replaces missing data with multiple plausible values, generating multiple datasets for analysis and combining the results for inference.

# Next step

We will use mix-effect model as the major method for both dataset.

No way to know what was the original method used. But we will use simple method like LOCF to mimic for comparison
