...
independent from the observed baseline variables \(X1\) and \(X2\),
which are pre-randomised variables, and outcome variable \(Y\) (which
contains potentially missing data).
...
being observed is not dependent on the observed data only but on the
missing data also. This mechanism is the most difficult to deal with as
...
mechanisms. The following DAG includes the missing data indicator \(R\).
In this structure, the potentially missing outcome variable \(Y\)
directly influences \(R\).
