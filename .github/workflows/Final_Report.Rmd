...
```{r fig.width=5, fig.height=4, echo=FALSE, fig.cap="MCAR DAG"}
MCAR_dag
```
...
```{r fig.width=5, fig.height=4, echo=FALSE, fig.cap="Missing at Random conditional on L, X_1 and X_2"}
MAR_dag1
```

```{r fig.width=5, fig.height=4, echo=FALSE, fig.cap="Missing at Random conditional on L, X_1"}
MAR_dag2
```
...
```{r fig.width=5, fig.height=4, echo=FALSE, fig.cap="Missing not at Random conditional on L, X_1 and X_2"}
MNAR_dag1
```

```{r fig.width=5, fig.height=4, echo=FALSE, fig.cap="Missing at Random conditional on L, X_2"}
MNAR_dag2
```
\newpage
...
```{r fig.width=5, fig.height=3, fig.cap="The distribution of sex in both treatment groups of the acupuncture trial data. The participants are predominantly female."}
print(acu_sex_plot)

```

```{r fig.width=5, fig.height=3, fig.cap="The distribution of age in both treatment groups. Both distributions are negatively skewed, predominantly middle aged and older."}
print(acu_age_plot)
```
...
*Table 6* shows the summary statistics of partial VITAL data. It provides the mean value (standard deviation) of the continuous variables and the count(%) for the categorical variables. This is divided per treatment combination group. Note here that the VITAL data
...
```{r fig.width=5, fig.height=3, fig.cap="Non-monotonic pattern across out knee pain scores in VITAL data."}
...
```{r fig.width=5, fig.height=3, fig.cap="Percentage of missing data across baseline and 4 follow-up time points. Non-monotonic pattern appears similar in 4 randomised groups."}
...
```{r fig.width=6, fig.height=9.5, fig.cap="Missing data patterns across 4 treatment groups in VITAL data."}
