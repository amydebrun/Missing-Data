...
bibliography: Missing_data_reference.bib
biblio-style: unsrt
...
models. This is not due to a limitation of CCA or linear mixed effects itself, but
...
Complete Case Analysis (CCA), Last Observation Carried Forward (LOCF),
...
point in the treatment group is *13.71828* and *17.01089* in the control
group. The standard deviation of the knee pain score at the final time
point in the fishoil group is *19.13955*, *19.24856* in the vitamin D
group, and *18.29479* in the control group. These statistics are based
...
```{r fig.width=5, fig.height=3, fig.cap="Acupuncture data set analysed with different methods; CCA = complete case analysis; LOCR = last observation carry forward; MO = mean observation; MI = multiple imputation with linear regression; LME = linear mixed-effects model without imputation; MI+LME = multiple imputation followed by linear mixed-effects model."}
...
$K=5$, $K=20$, and $K=100*\lambda$ (Which is 25 and 50 for Acupuncture
...
-   In the VITAL dataset, although there are more timepoints, baseline
    ...
    incorporate auxiliary post-randomisation information during the
