...
```{r fig.width=4, fig.height=3, warning=FALSE, fig.cap="The distribution of males and females in the VITAL data. Similar to the acupuncture trial, the participants are predominantly female."}
...
```{r fig.width=5, fig.height=3, fig.cap="Proportion of missing data (dark green) in outcome variables $pk2$ and $pk5$. This follows a monotonic missing data pattern."}
...
```{r fig.width=5, fig.height=3, fig.cap="Higher percentage of missing data in control group"}
...
```{r fig.width=5, fig.height=3, fig.cap="Missing data patterns across 4 treatment groups in VITAL data."}
...
Deciding which δ values are suitable is subjective. We need to include δ
= 0 as that assumes the data is MAR which acts as our baseline, and we
add the non-zero δ values to test the sensitivity to the MNAR
assumption. We decided to base our δ values on the standard deviations
of the outcome variables of interest. The standard deviation of the
headache severity score at the final time point in the treatment group
is *13.71828* and *17.01089* in the control group. The standard
deviation of the knee pain score at the final time point in the fishoil
group is *19.13955*, *19.24856* in the vitamin D group, and *18.29479*
in the control group. These statistics are based on the data in the
original wide format. We also extracted the standard deviation of the
outcome variables in the long format in which the estimand is continous.
We decided an increasing and decreasing multiples of these values should
suffice;

$\delta$ = -0.5*SD, -0.25*SD, 0 , 0.25*SD, 0.5*SD\$

Applying these to both datasets, our chosen δ values are;
