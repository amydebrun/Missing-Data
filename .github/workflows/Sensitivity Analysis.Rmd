...
for (i in seq_along(imp.all.undamped_cont)) {
  ...
  delta_results_cont <- bind_rows(delta_results_cont, est_cont)
}

delta_results_cont$group<-"Acupuncture Treatment"
ggplot(delta_results_cont, aes(x = estimate, y = delta)) +
   geom_point(size = 4, color = "#a80050") +
  geom_errorbarh(aes(xmin = conf.low, xmax = conf.high), height = 0.4) +   
  geom_vline(xintercept = 0, linetype = "dashed", color = "red") +   
  facet_wrap(~ group) +
