...
data_miss %>%
  ...
  geom_col() + ggtitle("Pattern of Missing Data over time in Knee Pain trial") + 
...
```
```{r mytable2, echo=FALSE, warning=FALSE, fig.cap='Data Observed (dark purple), Missing Data (light purple)', fig.height=9, fig.width=15}
...
data_miss %>%
  ...
  ungroup %>%
  ggplot(aes(x = time, fill = miss_obs, y = count_pat)) +
  geom_col() + ggtitle("Proportions of missing data from baseline to year 4") +
